...
import type { Command, EquipmentPosition, ExpeditionOutcome, GameState, Item, OfflineSummary, SkillDefinition, TargetPolicy } from './simulation/types';
import './styles.css';

function formatOutcomeCause(outcome: ExpeditionOutcome): string {
  if (outcome.result === 'completed') return `All Rooms in ${outcome.areaName} were secured.`;
  if (outcome.result === 'withdrawn') return `The Expedition was withdrawn from ${outcome.areaName} before all Rooms were complete.`;
  return `Ari was defeated during Combat in ${outcome.areaName}.`;
}

function formatOutcomeResolution(outcome: ExpeditionOutcome, recoveryRemainingMilliseconds = outcome.recoveryMilliseconds): string {
  if (outcome.result === 'defeated') {
    return `Recovery: ${Math.ceil(recoveryRemainingMilliseconds / 1_000)}s; ${outcome.willRestart ? 'the Area will restart automatically' : 'automatic repeat is stopped'}.`;
  }
  return `No Recovery is required; ${outcome.willRestart ? 'the Area will restart automatically.' : 'automatic repeat is stopped.'}`;
}
...
export function App() {
  ...
  return <main className={`shell ${page === 'skills' ? 'skills-route' : 'expedition-route'}`}>
    ...
    <section className="panel review" aria-label="Review queue">
      ...
      {game.reviewQueue.length === 0 ? <p>Nothing needs review.</p> : game.reviewQueue.map((item, index) => {
        const lootDecision = item.startsWith('Item and Loot decision:');
        const lootIndex = game.reviewQueue.slice(0, index + 1).filter((entry) => entry.startsWith('Item and Loot decision:')).length - 1;
        const queuedItem = lootDecision ? game.inventory.filter((inventoryItem) => !game.reviewedItemIds.includes(inventoryItem.id))[lootIndex] : undefined;
        return <p key={`${item}-${index}`}>{item}{queuedItem && <> <a href="#inventory" onClick={() => setInspectedItemId(queuedItem.id)}>Review Loot</a></>}</p>;
      })}
    ...
    {game.outcome && game.status !== 'active' && <details className="panel detail-panel" open><summary>Expedition outcome</summary><section aria-label="Expedition outcome" aria-live="polite"><span className="label">EXPEDITION OUTCOME</span><h2>{game.outcome.result}</h2><p>Cause: {formatOutcomeCause(game.outcome)}</p><p>Room reached: {game.outcome.roomReached} · committed {game.outcome.committed.experience} XP and {game.outcome.committed.currency} currency</p><p>Lost from the incomplete Room: {game.outcome.lost.experience} XP and {game.outcome.lost.currency} currency.</p><p>Consumables used: {Object.entries(game.outcome.consumables.potionsUsed).map(([kind, count]) => `${count} ${kind} Potion${count === 1 ? '' : 's'}`).join(' · ') || 'none'}{game.outcome.consumables.timedBuff ? ` · timed buff: ${game.outcome.consumables.timedBuff}` : ''}</p><p>{formatOutcomeResolution(game.outcome, game.recoveryRemainingMilliseconds)}</p></section></details>}
    ...
    <section className="panel log" id="history" aria-label="Recent outcomes"><span className="label">RECENT EXPEDITION OUTCOMES</span><div className="log-entries" aria-live="polite">{game.outcomeHistory.length === 0 ? <p>No Expedition outcomes yet.</p> : game.outcomeHistory.slice().reverse().map((outcome, index) => <article key={`${outcome.areaName}-${outcome.result}-${index}`}><h3>Expedition {outcome.result} · {outcome.areaName}</h3><p>Cause: {formatOutcomeCause(outcome)}</p><p>Room {outcome.roomReached} · {outcome.committed.experience} XP and {outcome.committed.currency} currency committed · {outcome.lost.experience} XP and {outcome.lost.currency} currency lost</p><p>{formatOutcomeResolution(outcome)}</p></article>)}</div></section>
    {offlineSummary && offlineSummary.requestedMilliseconds > 0 && <section className="panel" aria-label="Offline Summary"><span className="label">OFFLINE SUMMARY</span><h2>{Math.floor(offlineSummary.elapsedMilliseconds / 60_000)}m {Math.floor(offlineSummary.elapsedMilliseconds % 60_000 / 1_000)}s offline progress</h2><p>{offlineSummary.completedRooms} Rooms completed · {offlineSummary.outcomes.completed} completed Expeditions · {offlineSummary.outcomes.withdrawn} Withdrawals · {offlineSummary.outcomes.defeated} Defeats · {offlineSummary.recoveryEvents} Recovery events</p><p>{offlineSummary.rewards.experience} XP and {offlineSummary.rewards.currency} currency committed; {offlineSummary.lost.experience} XP and {offlineSummary.lost.currency} currency lost from incomplete Rooms{offlineSummary.capped ? ' · work capped at eight hours' : ''}{offlineSummary.skippedMilliseconds > 0 ? ` · ${offlineSummary.skippedMilliseconds} ms skipped` : ''}.</p>{offlineSummary.outcomeDetails.map((outcome, index) => <p key={`${outcome.areaName}-${outcome.result}-${index}`}><strong>Expedition {outcome.result}:</strong> {formatOutcomeCause(outcome)} {outcome.committed.experience} XP and {outcome.committed.currency} currency committed; {outcome.lost.experience} XP and {outcome.lost.currency} currency lost. {formatOutcomeResolution(outcome)}</p>)}</section>}
    <section className="panel settings" id="settings" aria-label="Settings"><span className="label">SETTINGS</span><h2>Expedition preferences</h2><p className="muted">Automatic repeat restarts the selected Area after completion or Recovery. Turning it off during Combat lets the active Expedition finish and stops the next repeat. Changes are saved with your progress.</p><label><input type="checkbox" aria-label="Automatically repeat Expeditions" checked={game.autoRepeat} onChange={(event) => send({ type: 'SET_AUTO_REPEAT', enabled: event.target.checked })} /> Automatically repeat Expeditions</label><p>Controlled time: enabled</p><div className="save-controls" aria-label="Developer save tools"><h3>Developer save tools</h3><textarea aria-label="Save data" value={saveText} onChange={(event) => setSaveText(event.target.value)} placeholder="Export a save or paste one here" rows={4} /><div><button className="secondary" onClick={exportSave}>Export Save</button><button className="secondary" onClick={importSave} disabled={!saveText.trim()}>Import Save</button></div>{saveMessage && <p role="status">{saveMessage}</p>}</div></section>
  </main>;
}
